...
First off, thank you for taking the time to contribute! Contributions from the community help make k8s-pod-oom-oracle a robust and reliable diagnostics tool for everyone.
...
* `just build` — Compile the Go binary to `bin/k8s-pod-oom-oracle`.
...
1. **Error Formatting:**
   ...
   * Wrap errors with `%w` for context propagation: `fmt.Errorf("reading cgroup stats: %w", err)`.
2. **Testing Standards:**
...
### Example:
```
feat(ebpf): attach tracepoint to kernel oom_kill_process events
```
